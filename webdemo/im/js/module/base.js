...
function getMyUserInfo() {
    try {
        if (this.personlist[userUID]) {
            var user = this.personlist[userUID];
            ai_detail.myPic = getAvatar(user.avatar);
        }
        else {
            ai_detail.myPic = getAvatar("");
        }
    }
    catch(error){
        ai_detail.myPic = getAvatar("");
...
YX.fn.openChatBox = function (account, scene) {
    ...
    if (scene == 'p2p') {
        ...
        if (info.account == userUID) {
            ...
            this.$chatTitle.find('img').attr('src', '../../static/images/myPhone.png');
        } else {
