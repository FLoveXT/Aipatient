...
var SDKBridge = function (ctr, data) {
    var sdktoken = readCookie('sdktoken'),
        ...
        that = this;
    if (!sdktoken) {
        loadSetTimeOut("抱歉,读取错误,请重新进入房间", "/")
        return;
    }
    //缓存需要获取的用户信息账号
    this.person = {};
    ...
    window.nim = ctr.nim = this.nim = SDK.NIM.getInstance({
        ...
        //消息
        onmsg: ReceiveMessage.bind(this),
...
SDKBridge.prototype.getHistoryMsgs = function (param) {
    console.log("该方法被放置患端ai_detail.js中,by zev");
    return false;
 ...
 * 群成员静音
 */
SDKBridge.prototype.updateMuteStateInTeam = function (
    id,
    account,
    mute,
    callback
...
SDKBridge.prototype.updateMyInfo = function (
    nick,
    gender,
    birth,
    tel,
